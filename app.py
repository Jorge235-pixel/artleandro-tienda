...
def init_db():
    ...
    # Admin fijo inicial: asegura que en Render siempre funcione el acceso.
    # Usuario: admin / Contraseña: ArtLeandro2026, salvo que se definan variables de entorno.
    c.execute('SELECT COUNT(*) FROM admin WHERE usuario=?',(ADMIN_USER,))
    if c.fetchone()[0]==0:
        c.execute('INSERT INTO admin(usuario,password) VALUES(?,?)',(ADMIN_USER,generate_password_hash(ADMIN_PASS)))
    else:
        c.execute('UPDATE admin SET password=? WHERE usuario=?',(generate_password_hash(ADMIN_PASS),ADMIN_USER))
    ...
    # Productos iniciales automáticos. Busca imágenes tanto en static/uploads/productos como en static/.
    c.execute('SELECT COUNT(*) FROM productos')
    product_count=c.fetchone()[0]
    if product_count==0:
        imgs=[]
        for carpeta in [UPLOAD, os.path.join(APP_DIR,'static')]:
            if os.path.isdir(carpeta):
                imgs += [f for f in os.listdir(carpeta) if f.lower().startswith('producto_') and f.lower().endswith(('.jpg','.png','.jpeg','.webp'))]
        imgs=sorted(set(imgs))
        for i,img in enumerate(imgs,1):
            c.execute('INSERT INTO productos(nombre,descripcion,precio,stock,talla,color,imagen,coleccion_id,activo,destacado,creado) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
            (f'Prenda Art Leandro {i:02d}','Producto listo para editar desde el panel administrador.',35,10,'S,M,L,XL','Por definir',img,1,1,1 if i<=4 else 0,datetime.now().isoformat()))
    else:
        # Si ya existía una base vieja, activa productos para que no aparezca “No hay productos activos”.
        c.execute("UPDATE productos SET activo=1 WHERE activo IS NULL OR activo=0")
        c.execute("UPDATE productos SET stock=10 WHERE stock IS NULL OR stock<=0")
        c.execute("UPDATE productos SET precio=35 WHERE precio IS NULL OR precio<=0")
    db.commit(); db.close()
...
def static_image(filename):
    """Devuelve una ruta estática válida aunque las imágenes estén en static/ o static/uploads/productos/."""
    ...
    for rel in candidates:
        if os.path.exists(os.path.join(APP_DIR,'static',rel)):
            return url_for('static', filename=rel)
    return url_for('static', filename=filename)

def logo_url():
    for rel in ['logo.jpg','uploads/logos/logo.jpg','logo.png','uploads/logos/logo.png']:
        if os.path.exists(os.path.join(APP_DIR,'static',rel)):
            return url_for('static', filename=rel)
    return url_for('static', filename='logo.jpg')
...
def inject_settings():
    ...
    return dict(cfg=cfg, static_image=static_image, logo_url=logo_url)
